...
public class ReturnAdapter extends RecyclerView.Adapter<ReturnAdapter.MyPickupHolder> {
    Context mContext;
    deliveryCallback mListener;
    RiderPickupListModel model;

    public ReturnAdapter(Context context, deliveryCallback mListener) {
        mContext = context;
    ...
    public void onBindViewHolder(@NonNull MyPickupHolder holder, int position) {
       ...
       // holder.txtRider.setText(model.getShipments().get(position).getRider().getRider().getName());
        ...
        if (detail.getStatus()!=8 && detail.getReturnStatus()!="1" ){
